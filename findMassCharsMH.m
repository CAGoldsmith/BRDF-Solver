...
I_fem(1,1) = (1/12 * actuator.mass * (actuator.height^2 + actuator.length^2) +... %I about CoM for medial actuator 
    ...
    (segM(3) * norm(COMjr(2:3,3))^2)+...%parallel axis thm for whole segment 
...
I_fem(2,2) = (1/12 * actuator.mass * (actuator.hornWidth^2 + actuator.height^2) +... %I about CoM for medial actuator 
    ...
    1/12 * plasM(3) * (plasW(3)^2 + plasL(3)^2)^2 +... %I abt CoM for plastic
    plasM(3) * norm([actuator.hornWidth + plasCOM(1,3); 0])^2 +... %parallel axis thm for plastic
    1/12 * actuator.mass * (actuator.hornWidth^2 + actuator.height^2) +... %I abt CoM for lateral actuator
    actuator.mass * norm([actuator.hornWidth + plasL(3) + actuator.height/2; 0])^2) -... %parallel axis thm for lateral actuator
    (segM(3)* norm([COMjr(1,3); COMjr(3,3)])^2); %parallel axis thm for whole segment 
...
I_fem(3,3) = (1/12 * actuator.mass * (actuator.length^2 + actuator.hornWidth^2) +... %I about CoM for medial actuator 
    ...
    1/12 * plasM(3) * (plasL(3)^2 + plasH(3)^2) +... %I abt CoM for plastic
    plasM(3) * norm([actuator.hornWidth + plasCOM(1,3); plasCOM(2,3)])^2 +... %parallel axis thm for plastic
    1/12 * actuator.mass * (actuator.length^2 + actuator.height^2) +... %I abt CoM for lateral actuator
    actuator.mass * norm([actuator.hornWidth + plasL(3) + actuator.height/2; actuator.length/2])^2) -... %parallel axis thm for lateral actuator
    segM(3) * norm(COMjr(1:2,3))^2; %parallel axis thm for whole segment 
...
I_tib(3,3) = 1/12 * segM(4) * (plasL(4)^2 + plasH(4)^2) + actuator.I;
